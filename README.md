...
2. 启动应用后，菜单栏出现翻译图标（💬），图标右下角小圆点指示服务状态（绿：运行中，红：已停止），无 Dock 图标。
3. 点击菜单栏图标 → **启动服务**（或在"设置"中打开）。打开设置窗口时 Dock 图标出现，关闭窗口后消失，应用继续驻留菜单栏。
...
| 检查更新… | 检查并安装新版本（基于 GitHub Releases） |
| 设置… | 打开设置窗口（侧栏分页） |
| 退出 | 退出应用（服务随之停止） |

> 关于信息与应用版本号位于设置窗口侧栏最下方，该处也提供一键彻底退出按钮。
...
| 自动检查更新 | 开 | 定期经 appcast 检查新版本（可在菜单栏手动检查） |
| 接收测试版更新 | 关 | 开启后订阅 beta 通道的预发布版本 |
...
**如何更新到新版本？**
菜单栏点击“检查更新…”，或在设置“通用”页确认已开启“自动检查更新”。首次从 Release 下载的 zip 若被 Gatekeeper 拦截，右键应用选择“打开”即可；若仍被拦截，可移除隔离属性：`xattr -dr com.apple.quarantine /Applications/AppleMTDeepLX.app`。
