#!/usr/bin/env bash
# update-appcast.sh —— 向 appcast.xml 插入/替换一个发布条目（按 sparkle:version 去重，可重复执行）。
# 用法: update-appcast.sh --version V --url U --signature S --length L [--channel C] [--notes-file F]
set -euo pipefail

cd "$(dirname "$0")/.."

APPCAST="appcast.xml"
VERSION="" URL="" SIG="" LEN="" CHANNEL="" NOTES_FILE=""
...
[[ -n "$VERSION" && -n "$URL" && -n "$SIG" && -n "$LEN" ]] || die "缺少必需参数（--version/--url/--signature/--length）"
...
/<\/item>/ && skip {
    buf = buf "    </item>\n"
    # 去重：同版本旧条目丢弃，由新条目替换（匹配 enclosure 的 sparkle:version 属性）
    if (buf !~ ("sparkle:version=\"" ver "\"")) printf "%s", buf
    skip = 0; buf = ""; next
}
skip { buf = buf $0 "\n"; next }
/<\/channel>/ {
